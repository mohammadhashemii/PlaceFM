...
class TransAndInd:
    ...
    def __init__(self, data, dataset):
        ...
        self.labels_full = self.labels_full.reshape(-1)
    ...
    def to(self, device):
        ...
        self.region_adjacency = self.region_adjacency.to(device)
        self.region_area = self.region_area.to(device)
...
class DataGraphSAINT:
    ...
    def __init__(self, root, dataset, **kwargs):
        ...
        if not osp.exists(dataset_str): 
            ...
            url = 'https://drive.google.com/drive/folders/1VDobXR5KqKoov6WhYXFMwH4rN0FMnVOa'  # Change this to your actual folder URL
            ...
            if downloaded_folder and osp.isdir(downloaded_folder):
                # Recursively move all files from downloaded_folder (and any nested directories) to dataset_str
                for root, dirs, files in os.walk(downloaded_folder):
                    for file in files:
                        src_file = osp.join(root, file)
                        dst_file = osp.join(dataset_str, file)
        ...
        self.idx_train = torch.tensor(role['tr'])
        self.idx_test = torch.tensor(role['te'])
        self.idx_val = torch.tensor(role['va'])
        ...
        # load node_year attribute
        if dataset in ["ogbn-arxiv"]:
            self.node_year = pd.read_csv(dataset_str + 'node_year.csv', header=None)
