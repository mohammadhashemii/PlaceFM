...
class FSQ(InMemoryDataset):
    r"""
    POI dataset for geolocation learning from Foursquare data.
    ...
    def raw_file_names(self) -> str:
        return 'fsq_us.parquet'

    @property
    def processed_file_names(self) -> str:
        return f'{self.args.state}.pt'
    ...
    def process(self) -> None:
        args = self.args
        poi_raw_feats = extract_feats(args=args, path=self.raw_paths[0])
        import ipdb; ipdb.set_trace()
        assert len(poi_raw_feats) > 0, f"No POI records found within '{args.state}'. Please check the state name and try again."

        print(" 1. Extracting category embeddings...")
        num_levels = sum(1 for col in poi_raw_feats.columns if col.startswith("category_lvl"))
        emb = encode_category_levels(poi_raw_feats, level=num_levels, method="SD-CEM")
        ...
        localities = poi_raw_feats['locality'].tolist()
        y = torch.tensor([int(postcode) if postcode.isdigit() else -1 for postcode in postcodes], dtype=torch.long)
        
        print(f"2. Creating edges between POIs based on geographical distances using {args.edge_creation} algorithm...")
        ...
        region_areas = calculate_region_areas(unique_postcodes, shapefile_path=f'{args.load_path}/f-osm/Census/tl_2024_us_zcta520/tl_2024_us_zcta520.shp')
        ...
        region_adj = create_region_adjacency(y=unique_postcodes, shapefile_path=f'{args.load_path}/f-osm/Census/tl_2024_us_zcta520/tl_2024_us_zcta520.shp')
        ...
        category_levels = [f"category_lvl{i+1}" for i in range(num_levels)]
